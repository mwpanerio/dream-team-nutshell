...
function fx_get_image_tag( $image, $classes = '', string $size = 'full', bool $skip_lazy = false, array $atts = [] ): string {
    ...
    if( empty( $image_id ) ) {
        $image_id = get_field( 'placeholder_image', 'option' );
	}
...
/**
 * Get <img> tag for client logo
 *
 * The image for the client logo can be set in WP Admin > Theme Settings > Media Assets > Logo
 * 
 * Alt tag will automatically be set to site name
 *
 * @param   mixed   $classes    Either string or array of classes
 * @param   string  $size       Image size
 * @param   bool    $skip_lazy  Prevent WP Rocket from lazy-loading image
 ...
 * @return  string              If valid image, then <img> string; otherwise empty string
 */
function fx_get_client_logo( $classes = '', string $size = 'medium', bool $skip_lazy = true, array $atts = [] ) {
    $logo_id = fx_get_client_logo_image_id();

    if( !isset( $atts['alt'] ) || empty( $atts['alt'] ) ) {
        $atts['alt'] = get_bloginfo();
    }

    return fx_get_image_tag(
        $logo_id,
        $classes,
        $size,
        $skip_lazy,
